...
/**
    Função para ordenar a matriz em ordem decrescente
*/
void ordenarMatrizDecrescente(int n, int *matriz) {
    // Ordenar a matriz original em ordem decrescente
    for (int i = 0; i < n * n; i++) {
...
/**
    Função para criar e gravar a matriz tridiagonal decrescente
*/
void matrizD(int n, int *matriz) {
 // definirdados
    FILE *arquivo = fopen("MATRIZ2.TXT", "wt");
    int i = 0, j = 0;
    
 // Ordenar a matriz em ordem decrescente
    ordenarMatrizDecrescente(n, matriz);
    
 // mostrar e gravar a matrix decrescente
    printf("\n\n%s\n", "mostrar e gravar a matrix decrescente:");
    ...
    for(i = 0; i < n; i = i + 1){
        for(j = 0; j < n; j = j + 1){
            if(j <= i + 1 && j >= i - 1){
 // Matriz tridiagonal decrescente
                fprintf(arquivo, "%3d ", matriz[i + n * j]);
                printf("%3d ", matriz[i + n * j]);
            } else{
 // Elementos fora da matriz tridiagonal
...
void method_07() {
 ...
 // chamar função para mostrar e gravar a matriz
    matrizD(n, matriz);
...
/**
    função para verificar de existe potenciação por linha
*/
int verificarMatriz(int **matriz, int n){
    int i = 0, j = 0;
    
   for(i = 0; i < n; i = i + 1){
       for(j = 0; j < n; j = j + 1){
           if(matriz[i][j] != (int)pow(i + 1, j)){
               return 0;
           }
       }
   }
...
void method_08() {
 // definir dados
    FILE *arquivo = fopen("MATRIZ3.TXT", "rt");
    int n = 0, i = 0, j = 0;
    int result = 0;
 ...
 // chamar a função para verificar se a matriz do arquivo segue a potenciação por linha
    result = verificarMatriz(matriz, n);
    
 // respostas de acordo com o resultado que vier da função
    if(result == 0){
...
/**
    função para verificar de existe potenciação por linha
*/
int verificarMatrizC(int **matriz, int n){
    int i = 0, j = 0;
    
   for(i = 0; i < n; i = i + 1){
       for(j = 0; j < n; j = j + 1){
           if(matriz[j][i] != (int)pow(i + 1, j)){
               return 0;
           }
       }
   }
    ...
    return 1; 
} // end verificarMatrizC
/**
    Method_09
*/
void method_09() {
 // definir dados
    FILE *arquivo = fopen("MATRIZ4.TXT", "rt");
    int n = 0, i = 0, j = 0;
    int result = 0;
 ...
 // chamar a função para verificar se a matriz do arquivo segue a potenciação por coluna
    result = verificarMatrizC(matriz, n);
...
/**
    função para calcular a media e mostrar quais supermercados estao abaixo da media
*/
void calcularMedia(struct supermercado *supermercados, int n){
 // definir dados
    float soma = 0;
    int x = 0;
 ...
 // calcular a soma dos produtos
    for(x = 0; x < n; x = x + 1){
        soma = soma + supermercados[x].precoProduto;
    }
    
 // calcular a media do preço dos produtos
    float media = soma / n;
    
 // exibir a media dos produtos
    printf("%s%.2lf\n", "Preco medio do produto: ", media);
    
 // exibir os supermercados que estão abaixo da media
    printf("\n%s", "Supermercados com preco abaixo da media:\n");
    for(x = 0; x < n; x = x + 1){
        if(supermercados[x].precoProduto < media){
            printf("Supermercado: %s (Código: %d) - Preço: %.2f\n", supermercados[x].nome, supermercados[x].codigo, supermercados[x].precoProduto);
        }
    }
} // end calcularMedia
/**
...
void method_10(){
 // definir dados
    FILE *arquivo = fopen("DADOS3.TXT", "rt");
    int n = 0, x = 0;
    
 // identificar
    IO_id( "Method_10 - v0.0" );
    
 // ler o numero 'n' de supermercados
    fscanf(arquivo, "%d", &n);
    
 // alocar um array para struct supermercado
    struct supermercado *supermercados = malloc(n * sizeof(struct supermercado));
    
 // ler dados de supermercado do arquivo
    for(x = 0; x < n; x = x + 1){
        fscanf(arquivo, "%s %d %f", supermercados[x].nome, &supermercados[x].codigo, &supermercados[x].precoProduto);
    }
    
 // fechar arquivo
    fclose(arquivo);
    
 // chamar função para calcular a media
    calcularMedia(supermercados, n);
