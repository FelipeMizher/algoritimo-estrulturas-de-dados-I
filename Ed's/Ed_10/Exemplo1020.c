...
int** matrixAdd(int** matrix1, int rows1, int cols1, int** matrix2, int rows2, int cols2, int constante){
 // definir dados
    int i = 0, j = 0;
    int** soma = 0;

 // alocar um espaço na memoria
    soma = (int**)malloc(sizeof(int*) * rows1);

 // looping para somar os itens das matrizes
    for (int i = 0; i < rows1; i++) {
        soma[i] = (int*)malloc(sizeof(int) * cols1);
        for (int j = 0; j < cols1; j++) {
            soma[i][j] = matrix1[i][j] + constante * matrix2[i][j];
        }
    }
...
void method_19(){
 // definir dados
    ...
    int** matrix2;
    int rows1, cols1, rows2, cols2;
    int i = 0, j = 0, constante = 0;
    int** soma = 0;
    
  // identificar
    IO_id ( "Method_19 - v0.0" );

 // chamar a função para ler as duas matrizes
    matrix1 = readMatrixFromFile(fileName1, &rows1, &cols1);
    matrix2 = readMatrixFromFile(fileName2, &rows2, &cols2);
 ...
 // chamar a função para somar as matrizes
    soma = matrixAdd(matrix1, rows1, cols1, matrix2, rows2, cols2, constante);

 // condição para se nao tiverem o mesmo numero de linhas e colunas e exibir a soma em uma nova matrix
    if (rows1 != rows2 || cols1 != cols2) {
        ...
        for(i = 0; i < rows2; i = i + 1) {
            for(j = 0; j < cols2; j = j + 1) {
                printf("%d ", matrix2[i][j]);
            }
            printf("\n");
        }

        printf("Soma das matrizes com escala:\n");
        for(i = 0; i < rows1; i = i + 1) {
            for(j = 0; j < cols1; j = j + 1) {
                printf("%d ", soma[i][j]);
            }
...
int** matrixProduct(int** matrix1, int rows1, int cols1, int** matrix2, int rows2, int cols2){
 // definir dados
    int** product = (int**)malloc(sizeof(int*) * rows1);

 // looping para obter o rpoduto de duas matrizes
    for (int i = 0; i < rows1; i++) {
        product[i] = (int*)malloc(sizeof(int) * cols2);
...
void method_20(){
 ...
 // chamar função para ler os dois arquivos das duas matrizes
    matrix1 = readMatrixFromFile(fileName1, &rows1, &cols1);
    matrix2 = readMatrixFromFile(fileName2, &rows2, &cols2);

 // chamar outra função para calcular o produto das matrizes
    product = matrixProduct(matrix1, rows1, cols1, matrix2, rows2, cols2);

 // condição para se o numero de linhas e colunas firem diferentes e caso contrario, mostrar o resultado
        if (cols1 != rows2) {
...
// repetir até desejar parar
 ...
 {
 // identificar
 IO_id ( "EXEMPLO1020 - Programa - v0.0" );
 // ler do teclado
 IO_println ( "Opcoes" );
 IO_println ( "0 - parar" );
