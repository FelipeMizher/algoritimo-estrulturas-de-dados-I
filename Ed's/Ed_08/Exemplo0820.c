...
int acharQuantos(int procurado, int posicaoInicial, int n, int array[]) {
    int contagem = 0;
...
void method_20(){
 ...
 // pedir dados
    ...
    int contagem = acharQuantos(procurado, posicaoInicial, n, array);

    if (contagem > 0) {
        printf("O valor %d aparece %d vezes no arranjo a partir da posição %d.\n", procurado, contagem, posicaoInicial);
    } else {
        printf("O valor %d não foi encontrado no arranjo a partir da posição %d.\n", procurado, posicaoInicial);
    }
 // encerrar
